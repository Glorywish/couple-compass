    ...
    <div style={{ minHeight: "100vh", background: "#0f1729", color: "#f5f0e8", display: "flex", flexDirection: "column" }}>
      <div style={{ maxWidth: 520, margin: "0 auto", width: "100%", padding: "48px 24px", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        ...
        <button onClick={() => setLocation("/")} data-testid="button-back-home"
          style={{ background: "transparent", color: "rgba(245,240,232,0.5)", border: "none", cursor: "pointer",
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          ...
          <form onSubmit={handleJoin} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div>
              ...
              <input
                id="code" data-testid="input-session-code" value={code}
                onChange={e => setCode(e.target.value.toUpperCase())}
                placeholder={t.codePlaceholder} maxLength={8} autoFocus={!prefilledCode}
                style={{ ...inputStyle, fontFamily: "monospace", fontSize: "1.4rem", letterSpacing: "0.3em", textTransform: "uppercase" as const }}
              />
            </div>
            <div>
              <label style={{ display: "block", fontSize: "0.72rem", letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(245,240,232,0.45)", marginBottom: 8, fontWeight: 500 }}>
              ...
              <input
                id="name" data-testid="input-partner2-name" value={name}
            ...
            <motion.button type="submit" disabled={!canSubmit}
              ...
              style={{
                background: canSubmit ? "#7eaa92" : "rgba(126,170,146,0.25)",
                color: canSubmit ? "#0f1729" : "rgba(245,240,232,0.35)",
